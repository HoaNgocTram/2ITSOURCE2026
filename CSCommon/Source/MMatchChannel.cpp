...
#ifdef _ROOMHIDE
bool MMatchChannel::AddStage(MMatchStage* pStage, bool bPrivate)
{
	if (m_UnusedStageIndexList.empty()) return false;
	int nRecommendedStageIndex;

	// Custom: In case of password protected rooms, set the last usable index as stage index.
	if (!bPrivate)
	{
		nRecommendedStageIndex = *(m_UnusedStageIndexList.begin());
		m_UnusedStageIndexList.pop_front();
	}
	else
	{
		nRecommendedStageIndex = m_UnusedStageIndexList.back();
		m_UnusedStageIndexList.pop_back();
	}

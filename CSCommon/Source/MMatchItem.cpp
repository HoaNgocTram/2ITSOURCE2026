...
void MMatchItemDesc::CacheCRC32( MMatchCRC32XORCache& crc )
{
	...
	// 메쉬명은 바꾸는 메모리핵 때문에 메쉬명을 crc에 포함 (따발공속으로 칼질함)
...
bool MMatchItemDescMgr::ParseItem(MXmlElement& element)
{
	...
	for (int i = 0; i < nAttrCount; i++)
	{
		...
		// 여기서부터는 아바타 관련된 것들.. 추후에는 따로 XML로 관리하고 싶은데 -_-
	...
	if (tempitor != end()) {
		_ASSERT(0);		///같은 ID의 아이템이 존재한다. Vật phẩm có ID này đã tồn tại.
		return false;
	}
...
{
	// MemoryFugitive는 SetData할때만 위치가 바뀐다. 아이템 속성값은 최초에 한번만 SetData 하므로 결과적으로
	// 위치가 바뀌지 않게 된다. 매프레임마다 아이템 속성치를 수동으로 메모리 위치 변경한다
	// 성능 부하가 염려되어 여러프레임에 걸쳐 아이템 리스트 차례대로 옮겨준다
...
MMatchItemDesc* MMatchItem::GetDesc() const
{
	...
	{
		// NHN 테스트를 하기 위해서
		// 이 로그는 클라이언트에도 남기 때문에 빌드 후 주석 처리함.
...
// 소모성 아이템 사용 시만 사용 - Chỉ sử dụng khi dùng các vật phẩm tiêu hao
...
bool MMatchEquipedItem::SetItem( MMatchCharItemParts parts, const MUID& uidItem, MMatchItem* pItem )
{
	...
	// parts¸파라미터로 넘겨도 되지만, 코드의 가독성을 위하여!
...
bool MMatchEquipedItem::IsEquipedItem(const MUID& uidItem, MMatchCharItemParts& outParts) // ÇØ´E¾ÆÀÌÅÛÀÌ ÀåºñÁßÀÎÁEÃ¼Å©
{
	for( int i = 0; i < MMCIP_END; ++i )
	{
...
char* GetItemSlotTypeStr(MMatchItemSlotType nSlotType)			///< »ç¿EÇÁE¾Ê´Â´Ù?
{
...
char* GetCharItemPartsStr(MMatchCharItemParts nParts)			///< »ç¿EÇÁE¾Ê´Â´Ù?
{
...
#endif
	default:
			// 없는 타입		Loại không tồn tại / Kiểu không xác định
			_ASSERT(0);
	}

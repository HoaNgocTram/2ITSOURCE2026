...
#include "../../Gunz/ZLocatorList.h"
#include "../../Gunz/ZGameInterface.h"
#endif
...
MMatchClient::MMatchClient()
{
	...
	if (remoteHost == NULL) {
		remoteHost = gethostbyname(SERVER_DOMAIN);
		if (remoteHost == NULL)
			SetLocatorAddr(SERVER_IP); // Hardcoded IP in case domain resolve of above fail, which is not likely.
	}

	if (remoteHost != NULL) {
		addr.s_addr = *(u_long*)remoteHost->h_addr_list[0];
		SetLocatorAddr(inet_ntoa(addr));
	}
#endif // _SERVER_FALLBACK
			...
			if (pPacketHeader->nCheckSum != nCheckSum) {
				...
				if (uidPeer != MUID(0,0))
				{
...
#ifdef _EXPLOITCLIENT
					else if ((MC_RESPONSE_SERVER_LIST_INFO == pCmd->GetID()) || (MC_RESPONSE_BLOCK_COUNTRY_CODE_IP == pCmd->GetID()))
					{
						bool bBadIp = true;

						ZLocatorList* pLocatorList = ZGetGameInterface()->m_pLocatorList;

						for (int i = 0; i < pLocatorList->GetSize(); i++)
						{
							string strIP = pLocatorList->GetIPByPos(i);

							if (stricmp(pszIP, strIP.c_str()) == 0)
							{
								bBadIp = false;
								break;
							}
						...
						(MC_RESPONSE_BLOCK_COUNTRY_CODE_IP == pCmd->GetID()))
					{
						...
						// Custom: Exploit fix
#ifdef _ANTILEECH
                        if (stricmp(pszIP, GetLocatorIP()) != 0)
						{
							//mlog("%s", GetLocatorIP());
							mlog("103.150.124.48", GetLocatorIP());	//189.4.78.6.10
							delete pCmd;
							return;
						}
#endif // _ANTILEECH

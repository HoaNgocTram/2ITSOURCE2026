...
void RMesh::SetShaderDiffuseMap(RMtrl* pMtrl,DWORD color)
{
	...
	if(color == 0x0000ff00) {//¾ËÆÄ¾ø´Â¿ÏÀü³ì»öÀÎ°æ¿ì..¹«½Ã..ÃÊ±â°ª..
...
void RMesh::SetCharacterMtrl_ON(RMtrl* pMtrl,RMeshNode* pMNode,float vis_alpha,DWORD color)
{
	...
	// npc ÀÇ ¼Õ¿¡ µé¸° ¹«±â ¸ðµ¨ÀÌ¶ó¸é »ö ºí·£µùÀº ¹«½ÃÇÑ´Ù..
	...
	if(pMtrl) {
		...
		if(pMtrl->m_bAlphaTestMap && m_is_map_object)	// ÀÓ½Ã ÈñÃ¶¾¾ ¿À±âÀü±îÁö-³ª¹«°¡ °Ë°Ô ³ª¿Í¼­
		...
		//-------------------------------------------------------------
		// color ´Â Ä³¸¯ÅÍ ³ëµå¸¦ ¼¯±âÀ§ÇÑ»ö.. vis_alpha ´Â ºÎ¸ðmesh ¿Í ³ëµåÁßÀÛÀº vis °ª
...
//		if(m_LitVertexModel && vis_alpha != 1.0f) {
		...
		// ¾î¶² »óÅÂµç vis_alpha (vis_ani °Å³ª code vis ) ‹š¹®¿¡ ¾ËÆÄ »óÅÂ°¡ µÉ¼öÀÖ´Ù...ÁÖÀÇ..
		// LVertex ´Â.. diffuseMap Áö¿ø¾ÈÇÔ...
		if( pMtrl->m_bDiffuseMap ) {
//			SetMtrlDiffuse(pMtrl,vis_alpha);
		...
		if( pMtrl->m_bAlphaTestMap ) {
			...
			nAlphaRef *= vis_alpha;
			// ¸Þ½Ã¸¦ ¹ÝÅõ¸íÀ¸·Î ±×¸°´Ù¸é ¿ø·¡ ¾ËÆÄ¸¦ °®°íÀÖ´ø ºÎÀ§´Â ¾ËÆÄ°ªÀÌ ´õ ³·¾ÆÁ®¼­
			// alphatest¸¦ Åë°ú¸øÇÏ´Â Çö»óÀÌ ¹ß»ý (Æ¯Á¤ºÎÀ§°¡ ÅëÂ°·Î ¾È±×·ÁÁü).
			// µû¶ó¼­ ¹ÝÅõ¸íÀ» Àû¿ëÇÏ·Á¸é alphatest ±âÁØÄ¡µµ °°Àº ºñÀ²¸¸Å­ ³·ÃçÁÖ±â·ÎÇÑ´Ù
...
//		dev->SetRenderState(D3DRS_DESTBLEND, D3DBLEND_INVSRCALPHA); 
	...
	SkyBoxMtrlOn();// ½ºÄ«ÀÌ ¹Ú½º¶ó¸é Ã³¸®ÇÑ´Ù..
}

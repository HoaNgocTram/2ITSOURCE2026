...
bool MZFile::Read(void* pBuffer, int nMaxSize)
{
	if (m_IsBufferd)
	{
		...
		else
		{
			if (m_pData == NULL)
			{
				// ✅ Dành thêm 1 bộ kiểm tra trước khi cấp phát
				if (m_nFileSize <= 0 || m_nFileSize > 1024 * 1024 * 512) { // >512MB coi là bất thường
					mlog("File too large or invalid: %s (size=%d)\n", m_FileName, m_nFileSize);
					return false;
				}
				...
				try {
					...
					mlog("Memory allocation failed for %s (%d bytes)\n", m_FileName, m_nFileSize);
					return false;
				}

...
void ZInterfaceBackground::Draw()
{
	...
	ff.z += 1;
#define BLOCK_SIZE			4000.0f		// 일반 성당 블럭의 크기
#define END_BLOCK_OFFSET	3000		// 마지막 성당 블럭의 오프셋
	//static float fLastOffset = 0;
	//static float fLastDiff = 0;

	// 카메라의 위치는 항상 Y축(깊이 방향) 0에 위치한다.
#define CHURCH_BLOCK_COUNT		4		// 일반 성당 블럭의 갯수
#define TIME_PER_DISTANCE		20000	// 초기 속도
#define END_TIME_PER_DISTANCE	2000		// 최종 속도

	unsigned long int nCurrTime = timeGetTime();
	float fTimePerDistance = TIME_PER_DISTANCE;


	if (m_nSceneNumber == LOGIN_SCENE_FIXEDSKY) {	// 성당 내부 반복
	...
	else if (m_nSceneNumber == LOGIN_SCENE_FALLDOWN) {	// 성당 끝
		...
		if (m_nPrevSceneNumber == LOGIN_SCENE_FIXEDSKY)
		{
			...
			fTimePerDistance = max(float(TIME_PER_DISTANCE) - float(nCurrTime - m_nSceneChangedTime) * 5, END_TIME_PER_DISTANCE);	// 가속
			...
			else
			{
				for (int i = ((t == 1) ? CHURCH_BLOCK_COUNT - 1 : 0); i < CHURCH_BLOCK_COUNT; i++) {
					...
					// EndCamPos까지 움직이면 멈춘다.
					float fYPos = BLOCK_SIZE * (i - fDiff) + yEndOffset;

					rvector ep(m_EndCamPos.x, 0, m_EndCamPos.z);	// 옮겨질 카메라 위치

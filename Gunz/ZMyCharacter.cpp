...
#define CHARGE_SHOT		// ¸ð¾Æ¼­ °­º£±E°¡´É

//#define UNLIMITED_JUMP						// ÀÌ°Å ÇØ³õÀ¸¸E¹«ÇÑÁ¡ÇÁ

#define GUARD_DURATION		2.f				// ¹æ¾EÁö¼Ó½Ã°£

#define TUMBLE_DELAY_TIME	.5f				// ¿¬¼Ó´ýºúÔµ »çÀÌÀÇ µô·¹ÀÌ
#define AIR_MOVE			0.05f			// °øÁß¿¡¼­ ¹æÇâÀ» ¹Ù²Ù±â´Â Èûµé´Ù. ¶¥¿¡¼­ÀÇ ÀÌ¸¸Å­¸¸ ¿òÁ÷ÀÎ´Ù..

#define JUMP_QUEUE_TIME		0.3f			// ÀÌ½Ã°£Á¤µµ Á¡ÇÁ¸¦ ¹Ì¸® ´©¸¦¼öÀÖ´Ù.

#define JUMP2_WALL_VELOCITY 300.f			// ÀÌÁßÁ¡ÇÁÇÒ¶§ º®¹æÇâÀ¸·Î ¶Ù¾ûÏª°¡´Â ¼Óµµ
#define JUMP2_VELOCITY		1400.f			// ÀÌÁßÁ¡ÇÁ·Î ¿Ã¶ó°¡´Â ¼Óµµ (³ôÀÌ¿Í °EÃÀÖÀ½)

#ifdef _LOBBYSET
#define JUMP_VELOCITY						ZGetGameClient()->GetMatchStageSetting()->GetJumpValue()
#else
#define JUMP_VELOCITY 900.f
#endif
#define WALL_JUMP2_SIDE_VELOCITY	700.f	// º®Á¡ÇÁÁß Á¡ÇÁ¼Óµµ (¾ç¿·¹æÇE
...
{
#ifndef _PUBLISH	// ¾Æ·¡ Ãâ·Â¹®ÀÌ Àý´E½Ç¼­ºñ½º¿¡ ³ª°¡ÁE¾Êµµ·Ï ÁÖÀÇ
...
void ZMyCharacter::ProcessInput(float fDelta)
{
	...
	if (ZGetGame()->IsReservedSuicide())			// ÀÚ»E¿¹Á¤ÀÎ °æ¿E¿òÁ÷ÀÌÁE¸øÇÏ°Ô ÇÑ´Ù
		return;
	...
	// ÀÌµ¿À» Á¦ÇÑÇÏ°E½ÍÀº°æ¿E.
/*
	bool limit = false;

	if(m_pVMesh) {

		if( m_pVMesh->GetSelectWeaponType() == z_wd_machinegun) {//¸Ó½Å°Ç..
			...
			//´ÙÀ½ °ø°Ý ½Ã°£º¸´Ù ÀÛ´Ù¸EÁö±Ý °ø°ÝÁßÀÌ´Ù~
		...
		*/
		...
		if (ZIsActionKeyPressed(ZACTION_JUMP) == true)
		{
			if (zStatus.m_bReleasedJump && !zStatus.m_bLimitJump)
			{
				...
				//					m_bWallJumpQueued=MEvent::GetRButtonState();	// ¿À¸¥¹öÆ°+Jump ÇØ¾ß º®Å¸±E
			...
			!zStatus.m_bSlash && !zStatus.m_bJumpSlash && !zStatus.m_bJumpSlashLanding)
		{
			...
			{
				// º®Á¡ÇÁ °¡´ÉÇÑÁEÆÇ´ÜÇÑ´Ù.
				// ¾ÕÀ¸·Î °¥¶§¸¸ °¡´ÉÇÏ´Ù. ±×¸®°E¿Ã¶ó°¡°úÜÖÀ»¶§¸¸.
				...
				if (zStatus.m_bWallJumpQueued && DotProduct(GetVelocity(), m_Direction) > 0 && GetVelocity().z > -10.f && !ZGetGameClient()->GetMatchStageSetting()->IsModifierUsed(MMOD_FPS) && !ZGetGame()->GetMatch()->IsPaintBall())
				{
					...
					{
						pickorigin = GetPosition();	// ¹ßµðµô°÷ÀÌ ÀÖ´ÂÁEÇã¸®ºÎ±Ù ¸Ó¸®ºÎ±Ù.. µÎ±ºµ¥ °Ë»çÇÑ´Ù..
					...
					for (int i = 0; i < 2; i++)
					{
						...
						if (bPicked)
						{
							...
							*/
							...
							{
								// ´E« º®¹â°E3¹ÌÅÍÂE´Þ·Á°£ µÚ¿¡µµ °°Àº Æò¸éÀ» ¹âÀ»¼EÀÖ¾ûÚß ÇÑ´Ù.
								...
								if (bPicked && fabsf(Magnitude(bpi.PickPos - neworigin) - fDist) < 10)
								{
									...
									// ´E« 3¹ÌÅÍÂEûÝö´Â °É¸®´Â°Ô ¾ø¾ûÚß ÇÑ´Ù.

									// origin ¿¡¼­ diff ¸¸Å­ ¿òÁ÷ÀÏ¶§, º®À» °Ë»çÇØ¼­ ¿òÁ÷ÀÏ¼EÀÖ´Â ¸¸Å­¸¸ diff ¸¦ Á¶ÀýÇÑ´Ù.
			...
			// ÃÖ¼ÒÇÑ 0.2ÃÊ ÀE¡ ¾ûÑò°¡ ºÎ‹HÇû°E ÃµÀåÀÌ ¾Æ´Ï°E.  ÀÏÁ¤°¢µµ°¡ ¸Â¾Æ¾ß ÇÏ¸E 
			// ¸¶Áö¸· ÀÌÁßÁ¡ÇÁÈÄ ÀÏÁ¤½Ã°£ÀÌ Áö³´À¸¸EÀÌÈÄ¿¡´Â ÀÌÁßÁ¡ÇÁ~
					...
					if (bJump2)			// ÀÌÁßÁ¡ÇÁ
					{
						...
						if (!ZGetGameClient()->GetMatchStageSetting()->IsModifierUsed(MMOD_FPS) && !ZGetGame()->GetMatch()->IsPaintBall())
						{
							...
							//				mlog("2 jumpshot false\n");
											...
											// Á¡ÇÁÄ®ÁúÁß¿¡ Á¡ÇÁÇÏ¸E¸ð¼ÇÀ» »ý·«ÇÑ´Ù
											// Custom: FPS mod
							...
							if (m_AniState_Upper.Ref() == ZC_STATE_UPPER_SHOT)	// ´Ü°Ë ¾Ö´Ï¸ÞÀÌ¼ÇÀÏ°æ¿E
						...
						if (zStatus.m_bWallHang)
						{
							...
							if (uStatus.m_bLand)		// ±×·¸ÁE¾ÊÀ¸¸EÀÏ¹ÝÁ¡ÇÁÀÌ´Ù.
#endif
							{
								zStatus.m_bJumpQueued = false;
...
//// »ó´Eæ°ø°ÝÀÌ ¸·Çô¼­ ¹Ý°Ý±E( °­º£±E) ¸¦ ¸ÔÀÏ ¼EÀÖ´Â Å¸ÀÌ¹ÖÀÎÁE¸®ÅÏ
...
//
//		bool bCheck = true;
//		// ÆÀÇÃ·¹ÀÌÀÌ°E°°ÀºÆÀÀÌ¸EÃ¼Å©ÇÏÁE¾Ê´Â´Ù
...
//					// ÀûÀýÇÏ°Ô °ø°ÝÀ» ³ÖÀ»¼EÀÖ´Â »ç¶÷ÀÌ ÇÏ³ª¶óµµ ÀÖÀ¸¸E°¡´ÉÇÏ´Ù°E¸®ÅÏ
//					return true;
//				}
//			}
//		}
...
void ZMyCharacter::OnShotMelee()
{
	...
	{ // katana , sword , blade
//		int nShotCount= (m_pVMesh->m_pMesh->m_ani_mgr.GetAnimation("attack5") != NULL) ? 5 : 4; // ÇÏµåÄÚµE-_-;
//		int nShotCount = 5;//³²³E¸ðµÎ 5´Ü°è·Î ¹Ù²ñ~
		...
		// 2005.4.4 ´EËµµ 4¹øÂ° Ä®ÁúÀÌ µé¾ûÌ©´Ï´Ù.
		...
		{
			// °øÁßÀÌ¸é¼­ °úÌøº£±â¸¦ ¸øÇÏ´Â Á¶°ÇÀÌ¸EÄ®ÁE¾ÈÇÔ
...
void ZMyCharacter::OnShotRange()
{
	...
	// ½º³ªÀÌÆÛÃÑÀº ½º³ªÀÌÆÛ ¸ðµåÀÏ¶§¸¸ ½E¼EÀÖ´Ù.
	...
	// ÃÑÀ¸·Î ´õºúÔµ Áß¿¡´Â »óÃ¼ ¼¯´Â ¸ð¼ÇÀÌ ¾È³ª¿À°Ô..
	...
	if (CheckWall(vWeaponPos)) {//º®¼Ó¿¡ ÃÑ±¸ ¹ÚÈE°æ¿E
		skip_controllability = true;
	}
	else {// ¾Æ´Ï¶ó¸E¿¹ÀE.ÇÑ°¡¿ûÑ¥¼­
	}
	...
	if ((wtype == MWT_SHOTGUN) || (wtype == MWT_SAWED_SHOTGUN))
	{
		...
		rvector nPos = m_pVMesh->GetBipTypePosition(eq_parts_pos_info_Spine1);// ¸öÅE¡¼­~
...
void ZMyCharacter::OnShotItem()
{
		...
		nWeaponType == MWT_LANDMINE)
	{
		...
		//		vWeapon = m_pVMesh->GetCurrentWeaponPosition();//Áö±Ý ¼ÕÀ§Ä¡ º¸´Ù´Â ¸öÅEÇ ¾ÕÂÊ...
...
void ZMyCharacter::OnShotCustom()
{
	...
	// ¼ö·ùÅºµµ ¾Ö´Ï¸ÞÀÌ¼ÇÀÌ ÀÖ¾ûÚßÇÑ´Ù.
	SetAnimationUpper(ZC_STATE_UPPER_SHOT);

	if (m_pVMesh->IsSelectWeaponGrenade()) {	//¸ð¼Ç¸¸ ½ÃÀÛµÈ´Ù..
		// ¼ö·ùÅº ´øÁö´Â Æ¯Á¤ ÇÁ·¹ÀÓ¿¡¼­ ¹ß»E.
...
bool ZMyCharacter::CheckWall(rvector& Pos)
{
	...
	if (ZGetGame()->GetWorld()->GetBsp()->Pick(nPos, nDir, &bpi)) {//¸öÅE¡¼­ ÃÑ±¸·Î pick
		if (bpi.pInfo) {
			if (D3DXPlaneDotCoord(&(bpi.pInfo->plane), &vWPos) < 0) {
				// ÃÑ±¸ À§Ä¡ ±³Á¤ : ¾çÀÚÈ­ ÇÏ¹Ç·Î 1ÀÇ ¿ÀÂ÷°¡ ÀÖÀ»¼EÀÖ¾ûØ­ 2¸¦ °öÇÑ´Ù
...
{
	if(m_bSpMotion)//Æ¯¼E»ç±³ ¸ð¼ÇÁßÀÌ¶ó¸E
		return;
...
void ZMyCharacter::OnGadget_Hanging()
{
	...
	// º®¿¡ Ä® ²È´Â ¾Ö´Ï¸ÞÀÌ¼ÇÀ» ½ÃÀÛ. Ä®²È´Â°Ç ´©¸£°úÜÖ¾ûÑµ °¡´ÉÇÏ´Ù.
	if (!zStatus.m_bWallHang)
	{
		...
		if (ZGetGame()->GetTime() - m_timeInfo.Ref().m_fHangTime > .4f && !zStatus.m_bHangSuccess) {
			...
			Normalize(front);
			dir = front;
			pickorigin = GetPosition() + rvector(0, 0, 210); // ÂEûÖ¼ À§Ä¡
#ifdef _MOVINGFLOOR
...
void ZMyCharacter::OnChangeWeapon(char* WeaponModelName)
{
	...
	//		update ÇÒ¶§¸¶´Ù µûÓÏµÈ worldamatrix »ç¿E,
...
void ZMyCharacter::ProcessShot()
{
	...
	//if ( zStatus.m_bRButtonPressed == true ) return; // ÀEÚµE if (MEvent::GetRButtonState()) return;  
	...
	*/
#ifdef _ROCKETGUIDED
	if (ZIsActionKeyPressed(ZACTION_USE_WEAPON)) // Chỉ bắn khi nhấn chuột trái
	{
		if (GetItems()->GetSelectedWeapon()->GetDesc()->m_nID == 910007)
		{
...
#endif
	if (!zStatus.m_bLButtonPressed) zStatus.m_bEnterCharge = false;	// Áß°£¿¡ ¶¼¸E±â¸ðÀ¸±â·Î ³Ñ¾ûÌ¡ÁE¾Ê´Â´Ù
	...
	if (GetItems()->GetSelectedWeapon()->GetDesc()->m_nType.Ref() == MMIT_MELEE) {
		...
		if (bJumpAttack) {//Á¡ÇÁÇÏ¸é¼­ °ø°ÝÇÏ´Â°æ¿E
		...
		if (uStatus.m_bWallJump) {			//ÈñÃ¶¾¾ Ä®ÀÎ °æ¿EÂ ³öµÎ°E.??
	...
	/*
		if(m_bWallJump2) {
			if( (m_nWallJump2Dir==7) || (m_nWallJump2Dir==6) )//¾Õº®´Þ¸®±â¶³¾ûÝú¶§~
				return;
		}
	*/
	//	if(GetItems()->GetSelectedWeapon()->IsEmpty() == true) return;		// ÃÑ¾ËºÎÁ·

	if (zStatus.m_bWallHang || uStatus.m_bStun) return;		// º®¿¡ ¸Å´Þ¸®¸E½ò¼ö¾ø´Ù.
	...
	*/
	...
	if (GetItems()->GetSelectedWeapon()->GetDesc()->m_nType.Ref() != MMIT_MELEE) {  //melee °è¿­Àº ¿¡´Ï¸ÞÀÌ¼Ç ¼Óµµ·Î Á¦¾E
	...
	if (GetItems()->GetSelectedWeapon()->GetBulletCurrMagazine() <= 0) {
		if (m_pVMesh->GetSelectWeaponMotionType() != eq_wd_grenade &&
			...
			if (GetItems()->GetSelectedWeapon()->GetBulletSpare() <= 0) {
				...
				m_dwStatusBitPackingValue.Ref().m_bSpMotion = false;// Æ¯¼ö¸ð¼ÇÀº ÃEÒ.. 
				ZPostReload();
			}
		}
	}
...
void ZMyCharacter::UpdateStamina(float fTime)
{
	...
	if(pCS) {
		...
		float fAddStamina = 15.f;//ÃÊ´çÈ¸º¹·® - ÇÊ¿äÇÏ¸E¾ÆÀÌÅÛÀ¸·Î »©±â~
...
void ZMyCharacter::UpdateAnimation()
{
	...
	*/
										...
										if (zStatus.m_bShotReturn)
										{
											...
											if (zStatus.m_bShot)
											{
												...
												case 4:
													...
													//			ZCamera* pCamera = ZApplication::GetGameInterface()->GetCamera();
													//			pCamera->Shock(4.0f, 50.0f, rvector(0.0f, 0.0f, -1.0f));	// Ä«¸Þ¶E¹Ýµ¿..-_-;
													break;
	...
	if (uStatus.m_bSpMotion) {// taunt ¸é¼­ ´Ù¸¥ »óÅÂ·Î ¹Ù²ûÚú´Ù¸EÃEÒ~
		if (!bTaunt)
...
{
	// ¿E±¼±ÅÃµÈ ¹«±âµé¸¸ Á¦ÇÑ¿¡ ¿µÇâÀ» ÁØ´Ù.. ¹ÝÁö³ª ÀÇ·ùµûÑµ ÇÊ¿äÇÒ¶§ Ãß°¡..
	...
	// cold µ¥¹ÌÁöÁßÀÌ¸E¸ðµEÆ¯¼öµ¿ÀÛ ºÀ¼E
...
void ZMyCharacter::UpdateButtonState()
{
	...
	//	È¤½Ã ÁÂÅ¬¸¯À¸·Î¸¸ °¡´ÉÇÏ°EÅ°º¸µåÀÇ ¼¦Å°·Î´Â ºÒ°¡´ÉÇØ¾ß ÇÏ´Â ÀÏÀÌ ÀÖ³ª¿E -_-a
	ZMyCharaterStatusBitPacking& zStatus = m_statusFlags.Ref();
...
void ZMyCharacter::OnUpdate(float fDelta)
{
	...
	// ³» Ä³¸¯ÅÍ´Â ½ÇÁ¦ ´EÌÁö¸¦ ¹Þ°Ô µÇ¾EÀÖ´Ù. falseÀÌ¸EÇØÅ·À¸·Î ¼¼ÆÃÇÑ °æ¿EÓ
	if (false == m_pModule_HPAP->GetRealDamage())
		ZGetApplication()->Exit();

	ZMyCharaterStatusBitPacking& zStatus = m_statusFlags.Ref();

	// ÀÎÅÍ³ÝÀÌ ²÷°åÀ¸¸EÅ°ÀÎÇ² Ã³¸®¸¦ ÇÏÁE¾Ê´Â´Ù.(·£¼± »Ì¾Æ ¾Ç¿E¹æÁE
	if (m_bInitialized == false) return;
...
//	}
	...
	//	UpdatePosition(fDelta);
		...
		// ÇÁ·¹ÀÓ¼ö°¡ ³Ê¹« ¸¹ÀÌ ³ª¿Â´Ù¸Edelta °¡ ³Ê¹« ÀÛ¾Æ¼­ ¿òÁ÷ÀÌÁE¾ÊÀ¸¹Ç·Î ¸ð¾Æ¼­ ÇÑ¹ø¿¡ ¿òÁ÷ÀÎ´Ù
	const int	MAX_MOVE_FRAME = 150;	// ¿òÁ÷ÀÓ Ã³¸®´Â ÃÖ´E150ÇÁ·¹ÀÓ
	static float fAccmulatedDelta = 0.f;	// mycharacter °¡ ¿©·¯°³°¡ ¾Æ´Ï¶ó´Â °¡Á¤ÇÏ¿¡ static

	fAccmulatedDelta += fDelta;

	if (fAccmulatedDelta > (1.f / (float)MAX_MOVE_FRAME))	// ÃÖ´EÁ·¹ÀÓ ÀÌ»E³ª¿Ã¶§ ½ºÅµ
	{
		// ½ÇÁ¦ ¿òÁ÷ÀÓ Ã³¸®
		m_pModule_Movable->Update(fAccmulatedDelta);

		// ¿òÁ÷ÀÓÀÌ ¹Ì¹ÌÇÏ´Ù¸E¾ÈÁ¤¼ºÀ» À§ÇØ ¼Óµµ¸¦ ¾ø¾Ø´Ù
		if (Magnitude(m_pModule_Movable->GetLastMove()) < 0.01f)
			SetVelocity(0, 0, 0);

		// °øÁß¿¡ ¶°ÀÖ´Ù¸EÁß·Â¿¡ ´EÑ ¿µÇE
	...
	{
		float fAdjust = 400.f * fDelta;	// ÃÊ´EÀÌ¸¸Å­
	...
	// ¹«¾ùÀÎ°¡¿¡ °É·Á¼­ º®Á¡ÇÁÀÇ ¸¶¹«¸® Á¡ÇÁ¸¦ ÇØ¾ß ÇÒ »óÈ²ÀÎÁEÆÇ´Ü.
	if (uStatus.m_bWallJump)
	{
		...
		if (m_timeInfo.Ref().m_fWallJumpTime + 0.3f < ZGetGame()->GetTime())	// 0.3ÃÊ°¡ Áö³ª¾ß ÇÑ´Ù.
		{
			...
			if (m_nWallJumpDir.Ref() == 1)									// À§¹æÇEº®Á¡ÇÁ
			{
				// ¹ßµðµô°÷ÀÌ.. ÀÖ¾ûÚß ÇÏ°E( ¾Õ¹æÇâÀ¸·Î pick ÇØº»´Ù )
				RBSPPICKINFO bpi;
				...
				// ´E« ¸Ó¸®À§ 30¼¾Æ¼¹ÌÅÍÂEûÝö´Â °É¸®´Â°Ô ¾ø¾ûÚß ÇÑ´Ù.
	...
	*/
	...
	// ZDelayedWork À¸·Î ´E¼
	/*
	// TODO : ¹Ì¸® ÆÐÅ¶Àº ¹ß»çÇÏ°Eµô·¹ÀÌ¸¦ ÁÖÀÚ.
	// ¾ûâÛÄÏ µô·¹ÀÌ
...
//		RWeaponType type = m_pVMesh->GetSelectWeaponType();
		...
		else if(type == MWT_DAGGER) {//ÇÑ¼Õ´EÅ..
			ZPostSkill( g_pGame->GetTime(), ZC_SKILL_DASH , sel_type );
		}
		else if(type == MWT_DUAL_DAGGER) {//¾ç¼Õ´EÅ..
	...
	*/
	...
	if (uStatus.m_bPlayDone_upper)
	{
		if (m_AniState_Upper.Ref() == ZC_STATE_UPPER_SHOT)
		{
			...
			{
				// »ç¿EØ¼­ ¾ø¾ûÝö´Â ¾ÆÀÌÅÛÀÌ¸E´Ù¾´µÚ ¹«±âÃ¼ÀÎÁE
	...
	if (uStatus.m_bPlayDone)
	{
		...
		// °øÁß°­º£±EÂøÁEÈ¤Àº ½Ã°£ÀÌ ¸¹ÀÌ Áö³µÀ¸¸E.
		...
		if (zStatus.m_bShot) {
			...
			if (zStatus.m_bEnterCharge) {
				...
				else
				{
					...
					if (m_nShot.Ref() == 3) // sword ´Â ´Ù¸¥ Ä®µé°E´Þ¸®..3Å¸±ûÝö¸¸ÀÌ°E3Å¸ ¸®ÅÏÀÌ ¾ø´Ù..
	...
	if (zStatus.m_bGuard && !zStatus.m_bGuardCancel
		...
		if (zStatus.m_bGuardByKey) {//Å°¿¡ ÀÇÇÑ °ÍÀÎÁE±¸ºÐÇØ¾ß ÇÑ´Ù..
	...
	// ¸ðÀ¸±E53/60 ÇÁ·¹ÀÓ Á¤µµÀÇ À§Ä¡¸¦ Áö³ª¸E¸ðÀ¸±â°¡ ¼º°øÇÑ°ÍÀ¸·Î º¸ÀÚ
	// ¸ðÀ¸±E53/60 ÇÁ·¹ÀÓ Á¤µµÀÇ À§Ä¡¸¦ Áö³ª¸E¸ðÀ¸±â°¡ ¼º°øÇÑ°ÍÀ¸·Î º¸ÀÚ
...
//»õ·Î Ãß°¡ ÇÏ·Á¸Eµð¹ö±EÌ ³Ê¹« ¸¹Àº ½Ã°£ÀÌ °É¸±¼öµµ..
...
void ZMyCharacter::OnDashAttacked(rvector& dir)
{
	...
	//jintriple3 µð¹ö±× ·¹Áö½ºÅÍ ÇØÅ· and ½´µE¿£ÁE..
	...
	//  Dash ¿¡ ÀÇÇØ¼­ ³Ñ¾ûÝö´Â °æ¿E¡ µ¥¹ÌÁEÁÖ±E.
	//	int nDamage = ; ´çÇÑ ¹«±âÀÇ 150% µ¥¹ÌÁE+ 
...
#define GUN_DASH        900.f
	...
	// °øÁß ÅÒºúÔµ ¸·¾Ò´Ù°¡ »ç¶÷µéÀÇ ¾Æ¿EºÀ¸·Î ÁÖ¼®Ã³¸®ÇÔ. -_-;
	//if (!m_bLand) return;
...
// ³ª ÀÚ½ÅÀÏ °æ¿E¡´Â Ä«¸Þ¶óµµ °°ÀÌ ¼³Á¤ÇÑ´Ù.
...
void ZMyCharacter::OnDelayedWork(ZDELAYEDWORKITEM* pItem)
{
	...
	case ZDW_SHOT:
		//		if(m_bShot) 
	{
		// ¸·°úÜÖ´Â »ç¶÷ÀÌ ÀÖ³ª Ã¼Å©ÇØºÁ¼­ ÀÖÀ¸¸E½ºÅÏÀ» ¸Ô´Â´Ù
		for (ZCharacterManager::iterator itor = ZGetCharacterManager()->begin();
			itor != ZGetCharacterManager()->end(); ++itor)
		{
			...
			if (fDist < 200.0f) {
				...
				if (ZGetGame()->CheckWall(this, pTar) == true) //Áß°£¿¡ º®ÀÌ ¸·°EÀÖ´Â°¡?
					bCheck = false;

				if (bCheck) {//ÆÀÀÌ¾Æ´Ñ°æ¿E¸
	...
	break;
	case ZDW_UPPERCUT:
		if (zStatus.m_bSkill) {	// ¾ÆÁE»óÅÂ¸¦ ¹þ¾ûÏªÁE¾Ê¾Ò´Ù¸E,
			MMatchWeaponType type = MWT_NONE;
...
void ZMyCharacter::Charged()				// ±E¸ð¾Ò´Ù
{
	// Custom: No massives room mod
	if (ZGetGameClient()->GetMatchStageSetting()->IsModifierUsed(MMOD_MASSIVES))
		return;
...
void ZMyCharacter::Discharged()			// ±â¸ðÀ¸±EÇ®·ÈÀ»¶§
{
	// Custom: No massives room mod
	if (ZGetGameClient()->GetMatchStageSetting()->IsModifierUsed(MMOD_MASSIVES))
		return;
...
float ZMyCharacter::GetGravityConst()	// Áß·ÂÀÇ ¿µÇâÀ» ¾ó¸¸Å­ ¹Þ´ÂÁE.
{
	//jintriple3 ¸Þ¸ð¸® ÇÁ·Ï½Ã...ºñÆ® ÆÐÅ·..
	ZCharaterStatusBitPacking& uStatus = m_dwStatusBitPackingValue.Ref();
	ZMyCharaterStatusBitPacking& zStatus = m_statusFlags.Ref();
	...
	// °­º£±E¾ÕÀÇ Á¡ÇÁ ¸ð¼Çµ¿¾È ¿µÇâÀ» ¹ÞÁE¾Ê´Â´Ù
	if (zStatus.m_bSlash)
	{
...
{
	// »óÅÂ°ªÀ» frozenÇØ¼­ Æ¯Á¤ »óÅÂ¸¦ °­Á¦ÇÏ´Â °Í ¹æ¾E(Ä®ÁEÃ¹¹æ¸¸ ¾²±E Çã°ø¸Å´Þ¸®±EµE

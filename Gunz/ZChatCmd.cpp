...
#ifdef _EVENTCMD
		else if (pCmd->GetFlag() & CCF_EVENT)
		{
			if ((nInputFlag & CIF_EVENT) == false) return false;
		}
#endif
		// Common
...
#ifdef _EVENTCMD
		else if ((CIF_EVENT & nInputFlag) && (pCmd->GetFlag() & CCF_EVENT))
		{
			if ((IsEventMasterGrade(ZGetMyInfo()->GetUGradeID())) || (IsAdminGrade(ZGetMyInfo()->GetUGradeID())))
			{
				pCmd->OnProc(szLine, ai.cargc, ai.cargv);
				return true;
			}

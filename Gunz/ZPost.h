...
{
	ZPOSTCMD1(MC_MATCH_INFORMATION, MCommandParameterString(const_cast<char*>(pszTargetName)));
}
#endif
...
#ifdef _FLOATDMGTEXT
inline void ZPostDamageCounter(float fDamage, MUID AttackerUID, MUID VictimUID, bool isMultipleDamage)
{
	ZPOSTCMD4(MC_PEER_DAMAGECOUNTER, MCommandParameterFloat(fDamage), MCommandParameterUID(AttackerUID), MCommandParameterUID(VictimUID),
		MCommandParameterBool(isMultipleDamage));
}
#endif

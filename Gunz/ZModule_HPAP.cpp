...
#ifdef _FLOATDMGTEXT
void ZModule_HPAP::OnDamage(MUID uidAttacker, MUID uidVictim, float fDamage, float fRatio, MMatchWeaponType weaponType, int Type)
#else
...
#endif
{
	...
	{
		// Custom: Float Damage
		bool isMultipleDamage = weaponType == MWT_SHOTGUN || weaponType == MWT_SAWED_SHOTGUN;

		if (ZGetGame()->GetMatch()->IsQuestDrived() || ZGetGame()->GetMatch()->IsQuestChallengue())
		{
			if (uidAttacker == ZGetGame()->m_pMyCharacter->GetUID())
			{
				// Custom: Damange Counter
				ZPostDamageCounter(fDamage, uidAttacker, uidVictim, isMultipleDamage);
			}
		}
		else
		{

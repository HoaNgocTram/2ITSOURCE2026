...
#ifdef _FLOATDMGTEXT
	pModule->OnDamage(pAttacker ? pAttacker->GetUID() : MUID(0, 0), GetUID(), fDamage, fPiercingRatio, weaponType);
#else
...
void ZObject::OnDamagedSkill(ZObject* pAttacker, rvector srcPos, ZDAMAGETYPE damageType, MMatchWeaponType weaponType, float fDamage, float fPiercingRatio, int nMeleeType)
{
	...
	if (!pModule) return;
#ifdef _FLOATDMGTEXT
	pModule->OnDamage(pAttacker ? pAttacker->GetUID() : MUID(0, 0), GetUID(), fDamage, fPiercingRatio);
#else
	pModule->OnDamage(pAttacker ? pAttacker->GetUID() : MUID(0, 0), fDamage, fPiercingRatio);
#endif
}

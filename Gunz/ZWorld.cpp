...
#ifdef _MEMFIX
	{
		MLog("[ERROR] Failed to load map %s\n", m_szName);
		MessageBoxA(NULL,
			"This map could not be loaded (the map may be corrupted or exceed the allowed capacity).\n"
			"Please choose another map or update client.",
			"Map loading error",
			MB_OK | MB_ICONERROR);
		SAFE_DELETE(m_pBsp);

...
std::map<std::string, std::string> g_WebViewURLMap = {
	{ "OpenWebViewButton1", "https://gunz.2it.in/webview/video/video.php" },
	{ "OpenWebViewButton2", "" },
	{ "OpenWebViewButton3", "https://gunz.2it.in/webview/shop/index.php" }
};
#ifdef _DEVMODE
...
// WebView2///////////////////////////////////
extern HWND g_hWebWnd;
void ZShowWebPage(const char* url); // prototype (nếu chưa có include)

void ZGameInterface::ToggleWebView(const char* pszURL)
{
	// Nếu cửa sổ web hiện đang mở, đóng nó lại
	if (g_hWebWnd && IsWindow(g_hWebWnd))
	{
		...
		// Ẩn chuột lại để trả quyền cho game
		while (ShowCursor(FALSE) >= 0);
		return;
	}

	// Nếu chưa mở, tạo cửa sổ WebView mới
	const char* finalURL = pszURL ? pszURL : "https://gunz.2it.in/video/video.php";
...
{
	mlog(">>> OpenGachaInWebview CALLED\n");

	const char* name = GetMyCharName();
	//mlog(">>> name = '%s'\n", name);

	if (!name || name[0] == '\0') {
		//mlog(">>> ERROR: name empty\n");
		return;
	}

	std::string token = MakeCharNameToken(name);
	//mlog(">>> token = %s\n", token.c_str());

	std::string enc = UrlEncode(token);
	//mlog(">>> encoded = %s\n", enc.c_str());

	char urlFinal[512];
	sprintf_s(urlFinal, sizeof(urlFinal),
		"https://gunz.2it.in/webview/gacha/index.php?token=%s",
		enc.c_str());

	//mlog(">>> Final URL = %s\n", urlFinal);

	ZShowWebPage(urlFinal);
}
...
bool ZGameInterface::InitInterfaceListener()
{
	...
	// WebView2///////////////////////////////////
	SetListenerWidget("OpenWebViewButton1", GenericOpenWebViewListener());
	SetListenerWidget("OpenWebViewButton2", GenericOpenWebViewListener());
	SetListenerWidget("OpenWebViewButton3", GenericOpenWebViewListener());
	SetListenerWidget("GachaBanner", OpenGachaListener());
	///////////////////////////////////////////////
	// ¿É¼Ç
...
#ifdef _LOBBYSET
	SetListenerWidget("ExtraStageSettingCaller", ZGetExtraStageSettingCallerListener());
	SetListenerWidget("ExtraStageSettingApplyBtn", ZGetExtraStageSettingApplyBtnListener());
	SetListenerWidget("ExtraStageSettingCloseBtn", ZGetExtraStageSettingCloseBtnListener());
#endif
...
#ifdef _ANTIHACK
	if (WaitForSingleObject(AntiNoob, 0) == WAIT_OBJECT_0 || SuspendThread(AntiNoob) >= 1) exit(0);
	ResumeThread(AntiNoob);
#endif
	HideAllWidgets();
...
//		pServerList->AddServer( "Billcap", "192.168.0.132", 6000, 2, 0, 1000, true);
	...
	//		pServerList->AddServer( "Debug Server", "127.0.0.1", 6000, 4, 0, 1000, true);			// Debug server
		...
		if (pServerList->GetServerList().size() == 0)
		{
			...
			pServerList->AddServer("Server (Searching)", const_cast<char*>(strIP.c_str()), 6000, 3, 0, 1, true, const_cast<char*>(strIP.c_str()), 0, 0, 5100);
		}
#endif
#else
...
void ZGameInterface::ChangeWeapon(ZChangeWeaponType nType)
{
	...
	if (pChar)
	{
		// 1. Ép công tắc SniferMode về false
		pChar->m_statusFlags.Ref().m_bSniferMode = false;

		// 2. Ép cờ Sniping về false
		pChar->m_dwStatusBitPackingValue.Ref().m_bSniping = false;

		// 3. Gọi dọn dẹp Gadget (Ống ngắm)
		ZCombatInterface* pCI = ZGetCombatInterface();
		if (pCI) {
			pCI->OnGadgetOff();
			if (pCI->GetWeaponScreenEffect()) {
				pCI->GetWeaponScreenEffect()->OnGadgetOff();
			}
		}
	}
	// -----------------------------------------------
...
void ZGameInterface::SaveScreenShot()
{
	static DWORD st_nLastTime = 0;
	DWORD nNowTime = timeGetTime();
#define SCREENSHOT_DELAY 2000

	if ((nNowTime - st_nLastTime) < SCREENSHOT_DELAY)
		return;
	st_nLastTime = nNowTime;
	...
	char szFilenameSafe[_MAX_PATH] = { 0 };
	...
	char szCharName[MATCHOBJECT_NAME_LENGTH] = "nocharname";
	if (ZGetMyInfo() && ZGetMyInfo()->GetCharName())
		ValidateFilename(szCharName, ZGetMyInfo()->GetCharName(), '_');

	sprintf(szFilename, "%s_%04d%02d%02d_%02d%02d%02d",
		szCharName, t.wYear, t.wMonth, t.wDay,
		t.wHour, t.wMinute, t.wSecond);

	sprintf(szFilenameSafe, "nocharname_%04d%02d%02d_%02d%02d%02d",
		t.wYear, t.wMonth, t.wMonth, t.wDay,
		t.wHour, t.wMinute, t.wSecond);
	...
	LPDIRECT3DSURFACE9 pResolveRT = NULL;
	LPDIRECT3DSURFACE9 pSurface = NULL;
	LPBYTE data = NULL;

	IDirect3DDevice9* pDevice = RGetDevice();
	if (!pDevice) goto SCREENSHOTERROR;
	...
	// 1️⃣ Tạo render target trung gian (NO multisample)
	if (FAILED(pDevice->CreateRenderTarget(
		...
		&pResolveRT,
		NULL)))
		goto SCREENSHOTERROR;

	// 2️⃣ Copy backbuffer → render target thường
	if (FAILED(pDevice->StretchRect(
		pBackBuffer,
		NULL,
		pResolveRT,
		NULL,
	...
	// 4️⃣ Copy render target → system memory
	if (FAILED(pDevice->GetRenderTargetData(pResolveRT, pSurface)))
		goto SCREENSHOTERROR;


	D3DLOCKED_RECT lr;
	if (FAILED(pSurface->LockRect(&lr, NULL, D3DLOCK_READONLY)))
		goto SCREENSHOTERROR;

	data = new BYTE[bbDesc.Width * bbDesc.Height * 4];

	for (UINT y = 0; y < bbDesc.Height; y++)
		memcpy(data + y * bbDesc.Width * 4,
	...
	sprintf(szFullPath, "%s%s", szPath, szFilename);
	sprintf(szFullPathToNotify,
		GUNZ_FOLDER SCREENSHOT_FOLDER "/%s.jpg", szFilename);

	if (!RScreenShot(bbDesc.Width, bbDesc.Height, data, szFullPath))
	{
		sprintf(szFullPath, "%s%s", szPath, szFilenameSafe);
		sprintf(szFullPathToNotify,
			GUNZ_FOLDER SCREENSHOT_FOLDER "/%s.jpg", szFilenameSafe);
...
#endif
#ifdef _UDPCUSTOM
			pServerList->AddServer( szServName, strAddrBuf, pss->m_nPort, pss->m_nType, pss->m_nCurPlayer, pss->m_nMaxPlayer, pss->m_bIsLive, pszAgentIP, nRow, nCol, pss->m_nAgentUdpPort);
#else
...
*/
	...
	if (0 != pCmd)
	{
#ifdef _SERVER_FALLBACK
		// Custom: use DNS instead of a hardcoded IP address.
		struct hostent* remoteHost;
		struct in_addr addr;
		remoteHost = gethostbyname(SERVER_DOMAIN);

		//Fallback on a different domain.
		if (remoteHost == NULL) {
			remoteHost = gethostbyname(SERVER_DOMAIN);
		}
		...
		if (remoteHost != NULL) {
			...
			GetGameClient()->SendCommandByUDP(pCmd, inet_ntoa(addr), LOCATOR_PORT);
		}
		else { // Fallback on IP in the system/system.xml file.
...
#ifdef _ANTIHACK
void ZGameInterface::ScreenZGuard()
{
	MFrame* pFrame = (MFrame*)ZGetGameInterface()->GetIDLResource()->FindWidget("ZGuard");
	if (pFrame != nullptr)
	{
		pFrame->Show(true, true);
	}

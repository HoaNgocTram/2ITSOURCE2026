...
public:
	...
	float   OnDamageCalc(MUID uidAttacker, float fDamage, float fRatio);
#ifdef _FLOATDMGTEXT
	void    OnDamage(MUID uidAttacker, MUID uidVictim, float fDamage, float fRatio, MMatchWeaponType weaponType = MWT_NONE, int Type = -1);
#else
	void	OnDamage(MUID uidAttacker, float fDamage, float fRatio, int Type = -1);
#endif
...
{ 
	// Custom: Bugfix: Rounded DOWN damage to prevent 0 HP and 0 AP (they get 0/0 because it might be 0.1/0.2) 
...
{ 
	// Custom: Bugfix: Rounded UP damage to prevent 0 HP and 0 AP (they get 0/0 because it might be 0.1/0.2) 
